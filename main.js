...
console.log("Strings to numbers", stringsToNumbs(strNums));

  // With the same numbers, find the sum of the even values
const sumEvens = () => {
    numbers = stringsToNumbs(strNums);
    let accumulator = 0;
    let evensArray = [];
...
console.log("Evens Sum:", sumEvens());
...
console.log(`index: ${atxIdx(nums)}, value: ${nums[atxIdx(nums)]}`);
  ...
  //using a higher order function, create an array of the unique 'weather_state_name' values of the weather array. Your function should return the following array ['Light Cloud', 'Heavy Cloud', 'Showers']
   weatherStates = weather.map((state) => {
...
console.log("Weather States:", weatherStates);
  ...
  const idealTempArr = weather.filter((temp) => temp.min_temp === 15.915);
  const idealTemp = idealTempArr[0].id;
  
  console.log("Ideal temp ID:", idealTemp);